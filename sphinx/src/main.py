...
""" Seuil de confiance pour la détection des objets avec YOLO"""
confThreshold = 0.5                    
""" Seuil de suppression non maximale (NMS) pour supprimer les détections redondantes"""
nmsThreshold = 0.4                                 
""" Largeur et hauteur d'entrée de l'image pour le modèle YOLO"""
inpWidth = 416                         
inpHeight = 416                           
""" Fichier contenant les noms des classes détectées par YOLO"""
...
def get_video_name(video_path):                    
    """
    Renvoie le nom du fichier vidéo en supprimant le chemin d'accès.

    Args:
        video_path (str): Chemin d'accès complet vers le fichier vidéo.

    Returns:
        str: Nom du fichier vidéo.
    """
    return os.path.basename(video_path)
                                                                             
def getOutputsNames(net):                          
    """
    Récupère les noms des couches de sortie du modèle YOLO.

    Args:
        net: Réseau YOLO.

    Returns:
        List: Liste des noms des couches de sortie.
    """
    try:                                                                
...
class VehicleClassifier(nn.Module):                                      
    """
    Classe de modèle de classification de véhicule basée sur ResNet-50.
    """
...
def preprocess_vehicle_region(vehicle_region):                          
    """
    Pré-traite une région d'image de véhicule pour la classification.

    Args:
        vehicle_region (PIL.Image or ndarray): Région d'image du véhicule.

    Returns:
        torch.Tensor: Image pré-traitée.
    """
    try:                                                        
        ...
        transform = transforms.Compose([                        
            ...
            transforms.ToTensor(),
            # Normalisation des valeurs de pixel pour ResNet
...
def matricule(frame, outs, width_factor=1.1, height_factor=1.0):                          
    """
    Détection de la plaque d'immatriculation dans une image.

    Args:
        frame (ndarray): Image d'entrée.
        outs (ndarray): Sorties de YOLO pour l'image.
        width_factor (float): Facteur d'agrandissement de la largeur de la boîte englobante.
        height_factor (float): Facteur d'agrandissement de la hauteur de la boîte englobante.

    Returns:
        Tuple: Tuple contenant la plaque d'immatriculation détectée et le pays prédit.
    """
    try:                                                                                  
...
class VehicleCounter:
    """
    Classe pour compter les véhicules et détecter les caractéristiques.

    Args:
        video_path (str): Chemin d'accès à la vidéo d'entrée.
    """
    ...
    def publish_video_end_message(self):
        """
        Publie un message indiquant la fin de la vidéo sur le serveur MQTT.
        """
        try:
    ...
    def publish_json_to_mqtt(self, json_data):
        """
        Publie les données JSON sur le serveur MQTT.

        Args:
            json_data (str): Données JSON à publier.
        """
        try:
    ...
    def process_video(self):
         """
         Traite la vidéo d'entrée pour détecter les véhicules et leurs caractéristiques.
         """
         try:
